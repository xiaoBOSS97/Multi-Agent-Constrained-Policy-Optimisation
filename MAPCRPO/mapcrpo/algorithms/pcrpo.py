...
from mapcrpo.utils.util import get_gard_norm, huber_loss, mse_loss
from mapcrpo.utils.popart import PopArt
from mapcrpo.algorithms.trpo import pcgrad, pcgrad_v1
from mapcrpo.algorithms.utils.util import check
from mapcrpo.algorithms.r_mappo.algorithm.r_actor_critic import R_Actor
...
class PCRPO():
    ...
    def trpo_step_reward(self, sample):
        ...
        # todo:matmul sequence
        expected_improve = -torch.dot(g_step_dir, reward_loss_grad).sum(0, keepdim=True)
        expected_improve = expected_improve.data.cpu().numpy()

        # compute full step
        # 1/2 x^T H x
        shs = 0.5 * (g_step_dir * self.fisher_vector_product(self.policy.actor,
                                                            obs_batch,
                                                            rnn_states_batch,
                                                            actions_batch,
                                                            masks_batch,
                                                            available_actions_batch,
                                                            active_masks_batch,
                                                            g_step_dir)).sum(0, keepdim=True)

        # sqrt((x^T H x) / (2 * delta))
        lm = torch.sqrt(shs / self.kl_threshold)

        # fullstep = sqrt(2 * delta / (x^T H x)) * x
        fullstep = g_step_dir / lm[0]
        ...
        for i in range(self.ls_step):
            ...
            new_params = params - fraction_coef * (fraction**i) * fullstep
            self.update_model(self.policy.actor, new_params)
            values, action_log_probs, dist_entropy, new_cost_values, action_mu, action_std = self.policy.evaluate_actions(
                share_obs_batch,
                obs_batch,
    ...
    def trpo_step_cost(self, sample):
        share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
        value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, \
        adv_targ, available_actions_batch, factor_batch, cost_preds_batch, cost_returns_barch, rnn_states_cost_batch, \
        cost_adv_targ, aver_episode_costs = sample
        ...
        # compute full step
        # 1/2 x^T H x
        shs = 0.5 * (b_step_dir * self.fisher_vector_product(self.policy.actor,
                                                            obs_batch,
                                                            rnn_states_batch,
                                                            actions_batch,
                                                            masks_batch,
        ...
        expected_improve = -torch.dot(b_step_dir, B_cost_loss_grad).sum(0, keepdim=True)
        expected_improve = expected_improve.data.cpu().numpy()
        ...
        for i in range(self.ls_step):
            ...
            # new_reward_loss = new_reward_loss.data.cpu().numpy()
            # new_reward_loss = -new_reward_loss
            # TODO: determin neg or pos
            new_cost_loss = new_cost_loss.data.cpu().numpy()
            loss_improve = new_cost_loss - cost_loss
            expected_improve *= fraction
            ratio = loss_improve / expected_improve
    ...
    def pcrpo_update(self, sample, aver_episode_costs_value, update_actor=True):
        ...
        # todo: actor update
        pre_actor_params = self.flat_params(self.policy.actor)
        rescale_constraint_val = (aver_episode_costs.mean() - self._max_lin_constraint_val) * (1 - self.gamma)
